...
export default function AllocateInSNConfirm({ navigation, route }: any) {
  ...
  const fetchDetails = useCallback(async () => {
    ...
    try {
      ...
      if (result.message === 'success' && result.data && result.data.length > 0) {
        ...
        let fpallet = result.data[0].palletCode || palletCode;
        let fbox = result.data[0].boxCode || boxCode;

        if (scanType === "sn") {
          setTitle(`SN: ${SN} | ${fmodel} ${fcolor}`);
        } else {
          setTitle(`Pallet: ${fpallet} Box: ${fbox} | ${fmodel} ${fcolor}`);
        }
  ...
  }, [allocateCode, palletCode, boxCode, SN, scanType, gs_factoryCode, BASE_URL, navigation]);
  ...
  const renderItem = ({ item }: { item: SNConfirmDetail }) => (
